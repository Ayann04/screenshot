...
def trigger_scrape(request):
    ...
    try:
        ...
        try:
            ...
            ss1 = driver.save_screenshot(f"screenshot-1.png")
            _create_status(new_run, "Please solve CAPTCHA #2 in the UI", pil_image=captcha_img_2, captcha_key=ss1)
            _create_status(new_run,"filling details district , dates , deed type",)
            time.sleep(5)
            ...
            time.sleep(5)
            ss2=driver.save_screenshot(f"screenshot-2.png")
            _create_status(new_run, "Please solve CAPTCHA #2 in the UI", pil_image=captcha_img_2, captcha_key=ss2)
        ...
        try:
            while True:  # Keep looping through all pages until no next button
                    ...
                    ss3 = driver.save_screenshot("screenshot-4.png")
                    _create_status(new_run, "Please solve CAPTCHA #2 in the UI", pil_image=captcha_img_2, captcha_key=ss3) 
                    time.sleep(5)
                    ...
                    for i in range(len(data_elements_2)):
                        ...
                        _create_status(new_run,f"Scraping started on page{i}",)  
            ...
            print(e) 
            _create_status(new_run,f"Scraping failed{e}",)
